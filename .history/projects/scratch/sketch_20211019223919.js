...
function setup() {
    ...
    for(let i = 0; i < cols; i++) {
        for(let j = 0; j < rows; j++) {
          ...
          distmap = map(distance, 0, dist(0, 0, canvasHeight/2, canvasWidth/2), 10, 20)
          
        //   pendulums[i][j] = new Pendulum(i*((canvasWidth+80)/cols), j*((canvasHeight+80)/rows), c/0.6, [255, 255*(j%2), 255*(j%2)]);
          pendulums[i][j] = new Pendulum(i*((canvasWidth+100)/cols), j*((canvasHeight+100)/rows)-20, distmap, [100, 255-7*i, 255-7*j]);
        }
    }
