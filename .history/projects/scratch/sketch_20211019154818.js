...
function setup() {
    ...
    for(let i = 0; i < cols; i++) {
        ...
        //   pendulums[i][j] = new Pendulum(i*((canvasWidth+80)/cols), j*((canvasHeight+80)/rows), c/0.6, [255, 255*(j%2), 255*(j%2)]);
          pendulums[i][j] = new Pendulum(i*((canvasWidth+80)/cols), j*((canvasHeight+80)/rows), c/0.6, [100, 255-7*i, 255-7*j]);
        }
    }
...
function draw() {
    ...
    for(let i =0; i < cols; i++){
        for(let j = 0; j < rows; j++) { 
            ...
            pendulums[i][j].color[0] = 255/2*sin(time/70) + 255/2; 
            pendulums[i][j].color[1] += 255/2*cos(time/70) + 255/2;
            pendulums[i][j].color[2] += 255/2*sin(time/70) + 255/2; 
