...
function getTile(x, y, terrainScales) {
  ...
  for (let i = 0; i < scales.length; i++) {
    ...
    if (v <= terrainScale) {   
      ...
      console.log(dist(spriteX, spriteY, x*tileSize+tileSize/2, yx*tileSize+tileSize/2))
      // collision test
      if ((i == 0 || i == 3) 
      && dist(spriteX, spriteY, x*tileSize+tileSize/2, yx*tileSize+tileSize/2) < (2*tileSize/3+spriteR)) {
        resetSketch()
      }
